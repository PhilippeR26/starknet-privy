import type { WalletApiGenerateUserSignerResponseType, WalletApiRequestSignatureInput } from "@privy-io/server-auth";
...
export async function getUserAuthorizationKey({
  ...
  const res: WalletApiGenerateUserSignerResponseType = await privy.walletApi.generateUserSigner({
    userJwt: userJwt,
  });
